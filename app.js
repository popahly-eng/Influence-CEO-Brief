(() => {
  ...
  const state = {
    section: "crisis",
    lastUrl: "",
    counts: { crisis: null, coverage: null, event: null, all: null }
  };
  ...
  function showToast(message, type = "success") {
    ...
    showToast._timer = setTimeout(() => { el.toast.hidden = true; }, 3500);
  }
  ...
  async function loadFormConfig() {
    try {
      const res = await fetch(CONFIG.FORM_URL, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ test: true }) });
      const data = await parseResponseSafely(res);
      if (!res.ok || data.success === false) return;
      // Config endpoint is kept for compatibility and future section labels.
    } catch (err) {
  ...
  async function saveItem() {
    ...
    const url = state.lastUrl || el.urlInput.value.trim();

    if (!headline || !summary || !isValidUrl(url)) {
      showError("Headline, summary, and a valid URL are required.");
      return;
    }
    ...
    try {
      ...
      if (!res.ok || data.success === false) {
        const detail = Array.isArray(data.errors) ? data.errors.join(", ") : (data.message || "Save failed.");
